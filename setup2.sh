...
install_deps() {
    ...
    step=$((step+1)); progress_bar $step $total "Comprobando apache2-utils (htpasswd)..."
    if ! command -v htpasswd &>/dev/null; then
        warn "htpasswd no encontrado. Instalando apache2-utils..."
        sudo apt update -qq
        sudo apt install -y apache2-utils -qq
        print "apache2-utils instalado"
    else
        print "htpasswd disponible"
    fi
...
# ─── Configurar Nginx (Basic Auth para Kibana) ───────────────────
setup_nginx() {
    header "Configurando Nginx (Basic Auth para Kibana)"

    local htpasswd_file="config/nginx/.htpasswd"
    local nginx_conf="config/nginx/nginx.conf"

    # Generar nginx.conf si no existe
...
server {
    ...
    location / {
        auth_basic "ALV-POT - Acceso Restringido";
...
EOF
        print "nginx.conf generado"
    else
        print "nginx.conf ya existe, conservando el actual"
    fi

    # Generar .htpasswd si no existe
    if [ ! -f "$htpasswd_file" ]; then
        echo ""
        echo -e "  ${BOLD}Configura las credenciales de acceso a Kibana:${NC}"
        echo -e "  ${DIM}(Se usarán para el Basic Auth de Nginx)${NC}"
        echo ""

        read -rp "  Usuario [admin]: " nginx_user
        nginx_user="${nginx_user:-admin}"

        while true; do
            read -rsp "  Contraseña: " nginx_pass
            echo ""
            read -rsp "  Confirmar contraseña: " nginx_pass2
            echo ""
            if [ "$nginx_pass" = "$nginx_pass2" ]; then
                break
            fi
            warn "Las contraseñas no coinciden, inténtalo de nuevo."
        done

        htpasswd -bc "$htpasswd_file" "$nginx_user" "$nginx_pass"
        print "Credenciales guardadas para usuario '${nginx_user}'"
    else
        print ".htpasswd ya existe, conservando credenciales actuales"
        warn "Para cambiar la contraseña ejecuta: htpasswd config/nginx/.htpasswd <usuario>"
    fi
